...
async function submit() {
  error.value = null
  if (!token.value) {
    error.value = t('resetPasswordTokenMissing')
    return
  }
  ...
  try {
    ...
    error.value = t('passwordResetFailed')
  } finally {
...
<template>
  <div class="min-h-screen flex items-center justify-center bg-background px-4">
    <div class="w-full max-w-sm space-y-6 bg-card border border-border rounded-lg p-6 shadow-sm">
      <h1 class="text-xl font-bold">{{ t('resetPasswordTitle') }}</h1>

      <div v-if="done" class="text-sm text-green-600" data-testid="reset-password-success">
        {{ t('passwordResetSuccess') }}
      </div>

      <form v-else class="space-y-4" @submit.prevent="submit">
        <div v-if="error" class="text-sm text-red-600" data-testid="reset-password-error">{{ error }}</div>
        <div>
